...
const Candidates = () => {
    const [candiadates, setCandidates] = useState([]);
    useEffect(() => {
        const getCandidates = JSON.parse(localStorage.getItem("candidates") || '[]');
        setCandidates(getCandidates);
    }, [])
    let filterData = candiadates.filter((candiadate) => candiadate.role.toLocaleLowerCase().includes('candidate'.toLocaleLowerCase()));
    ...
    return (
        <>
            ...
            <h2 style={{ fontSize: "28px", color: "white", textAlign: "center" }}> Total candiadates <span className="text-info"> {filterData?.length}</span> </h2>
